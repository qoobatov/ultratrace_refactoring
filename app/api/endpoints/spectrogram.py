...
from app.core.io.spectrogram_generator import (
...
async def get_spectrogram(
    ...
    dynamic_range: float = Query(90, ge=1),
    colormap: str = Query(
        "grayscale",
        description=f"One of: {', '.join(SUPPORTED_COLORMAPS)}",
    ),
    threshold: int = Query(
        0,
        ge=0,
        le=255,
        description="Отсекает слабый сигнал в фон. 0 — не отсекать.",
    ),
    ...
    if colormap not in SUPPORTED_COLORMAPS:
        raise HTTPException(
            status_code=400,
            detail=f"colormap must be one of: {', '.join(SUPPORTED_COLORMAPS)}",
        )
