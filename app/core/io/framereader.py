"""
Чистые ридеры ультразвуковых данных (DICOM, AAA .ult и т.д.)
Адаптировано из оригинального UltraTrace.
Не зависит от tkinter, только numpy, pydicom, PIL, matplotlib (без GUI).
"""
...
class DicomReader(FrameReader):
    def getFrameTimes(self):
        ...
        try:
            ...
            return [
                float(int.from_bytes(headers[i : i + 4], byteorder="little")) / 1000000
                - offset
...
class DicomImgReader(DicomReader):
    ...
    def load(self):
        logger.debug("DicomImgReader: reading dicom")
        dcm = dicom.dcmread(self.filename)
        logger.debug("DicomImgReader: copying pixels")
        pixels = dcm.pixel_array
        del dcm
        self.RGB = len(pixels.shape) == 4
        if self.RGB and pixels.shape[0] == 3:
            logger.debug("DicomImgReader: reshaping pixels")
            pixels = pixels.reshape(pixels.shape[1:] + [3])
        logger.debug("DicomImgReader: dumping pixels")
        ...
        if self.RGB:
            self.dtype = np.dtype(
                ("uint8", (self.shape[1], self.shape[2], self.shape[3]))
            )
            self.frame_size *= 3
        else:
...
class DicomPNGReader(DicomReader):
    ...
    def load(self):
        ...
        for f in range(frames):
            if f % 10 == 0 or f == frames - 1:
                logger.debug("Processing frame %d/%d", f + 1, frames)
    ...
    def getFrame(self, framenum):
        try:
            ...
            logger.error("file %s does not exist", self.png_name % framenum)
            return None
...
class ULTScanLineReader(FrameReader):
    ...
    def getFrame(self, framenum):
        ...
        ret = Image.frombytes(
            "RGB", canvas.get_width_height(), self.argb2rgb(canvas.tostring_argb())
        )
        plt.close(fig)
        return ret.crop((xmin, ymin, xmax, ymax))

    def argb2rgb(self, argb):
        argb_split = [argb[i] for i in range(len(argb))]
